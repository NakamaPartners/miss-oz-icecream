...
- **Fonts:** script headings = **Kaleidos** (`--font-script`), small script accents = **Lemongrass** (`--font-script-alt`, utility `font-script-alt`); files live in `public/fonts/`. Both are scripts — long body paragraphs stay on the readable serif on purpose. **Higante Black is in** — `public/fonts/Higante-Black.otf`, leads `--font-display` and `--font-groovy` (all big headlines/titles) with Shrikhand/Bagel Fat One as fallbacks.

- **"Meet Miss Oz" section** (`MeetOz.tsx`, `#oz`, after Story): the shop is named after her dog Oz, a tricolor Cavalier King Charles Spaniel. Real photos live in `public/images/oz-*.webp`; three fill the section's taped snapshots, plus small taped snapshots in Guestbook (in-flow below the ledger, right-aligned at md+) and Footer (flex row beside the big tagline). User wants these visible at ALL sizes — keep them in normal flow, never absolutely positioned over headers (earlier absolute versions either overlapped text or got hidden below xl and the user complained). Client may still want small ILLUSTRATED Oz accents near section headers later. Framer-motion is globally gated via `<MotionConfig reducedMotion="user">` in home.tsx — keep it when restructuring. Lemongrass/Kaleidos font files are demo versions (fontkong) — client should buy commercial licenses before launch.
