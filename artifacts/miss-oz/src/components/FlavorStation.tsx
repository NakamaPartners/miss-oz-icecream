...
export default function FlavorStation() {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) return;
      const { choice: c } = JSON.parse(raw) as { choice?: unknown };
  ...
  return (
    <section
      className="parlour-paper relative overflow-hidden py-[80px] md:py-[112px] px-[6vw]"
      style={{
    ...
    >
      ...
      <motion.div
        ...
        className="text-center mb-[52px] md:mb-[64px]"
      >
        <span className="inline-flex items-center gap-3 text-[11px] tracking-[5px] uppercase font-bold"
          style={{ color: FLAVOR.accent, opacity: 0.75 }}>
          <Sparkle size={11} delay={0} /> Flavor Lab <Sparkle size={11} delay={0.8} />
        </span>
      </motion.div>

      {/* ── Main two-column layout ─────────────────────────────────────── */}
      <div className="grid grid-cols-1 md:grid-cols-[minmax(0,340px)_1fr] gap-10 md:gap-[64px] items-start max-w-[1280px] mx-auto">

        {/* ════════════════════════════════════════════════
            LEFT — Poster (sticky anchor on desktop)
        ════════════════════════════════════════════════ */}
        <motion.div
          ...
          className="relative mx-auto w-[min(320px,80vw)] md:sticky md:top-[80px]"
        >
          {/* glow halo */}
            ...
            <div className="rounded-[4px] p-[6px]" style={{ background: 'var(--cream-hi)' }}>
              {FLAVOR.poster ? (
                ...
                <div className="w-full rounded-[2px] flex flex-col items-center justify-center gap-6 py-16 px-8"
                  style={{ minHeight: '320px', background: `linear-gradient(165deg, ${FLAVOR.accent}28 0%, #152F26 65%)` }}>
                  <div className="text-center leading-[1.1]"
                    style={{ fontFamily: "'Cookie', cursive", fontSize: 'clamp(28px,8vw,42px)', color: 'var(--cream-hi)' }}>
                    {FLAVOR.name}
                  </div>
                </div>
              )}
        ...
        {/* ════════════════════════════════════════════════
            RIGHT — One continuous column: reveal → vote
        ════════════════════════════════════════════════ */}
        <div className="flex flex-col text-left">

          {/* ── Zone A: Featured Flavor ── */}
          <div className="relative">
            <Sparkle size={18} delay={0}   className="hidden md:block absolute -top-3 right-8" />
            <Sparkle size={14} color="var(--berry)" delay={0.8} className="hidden md:block absolute top-6 -right-1" />
            ...
            >
              <Sparkle size={13} delay={0.2} /> New Flavor Alert <Sparkle size={13} delay={0.6} />
            </motion.span>
            ...
            <motion.h2
              ...
              className="text-[clamp(42px,5.2vw,68px)] leading-[0.96] mb-4 text-[var(--cocoa)]"
              style={macklin}
            ...
            <motion.p
              ...
              className="italic text-[17px] leading-relaxed text-[#1d0e0d] opacity-80 max-w-[420px]"
          ...
          {/* ── Inline Bridge ─ no divider, just a transition line ── */}
          <motion.div
            initial={{ opacity: 0, scaleX: 0 }} whileInView={{ opacity: 1, scaleX: 1 }}
            viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.3 }}
            className="my-[36px] md:my-[44px] flex items-center gap-4"
            style={{ transformOrigin: 'left' }}
          >
            <div className="flex-1 h-px" style={{ background: `linear-gradient(to right, ${FLAVOR.accent}50, ${FLAVOR.accent}18)` }} />
            <span className="font-script text-[20px] text-[var(--berry-deep)] opacity-70 select-none whitespace-nowrap">
              and next month's flavor? that's on you
            </span>
            <div className="flex-1 h-px" style={{ background: `linear-gradient(to left, ${FLAVOR.accent}50, ${FLAVOR.accent}18)` }} />
          </motion.div>

          {/* ── Zone B: Vote — no heavy heading restart ── */}
          <div id="vote" style={{ scrollMarginTop: '32px' }}>

            {/* Compact vote heading */}
            <motion.div
              initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ duration: 0.55 }}
              className="mb-[6px]"
            >
              <span className="font-script text-[var(--berry-deep)] text-[clamp(22px,2.4vw,30px)] leading-snug">
                you decide —
              </span>
            </motion.div>
            <motion.h3
              initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ duration: 0.55, delay: 0.07 }}
              className="text-[clamp(28px,3.5vw,46px)] leading-[0.98] mb-3 text-[var(--cocoa)]"
              style={macklin}
            >Vote for the Next Flavor</motion.h3>
            <motion.p
              initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }} transition={{ duration: 0.55, delay: 0.12 }}
              className="italic text-[15px] text-[#1d0e0d] opacity-70 mb-[28px]"
            >
              {revealed
            ...
            {/* Vote cards — fill the right column width */}
              ...
              {CARDS.map((card, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bulbframe relative rounded-[14px] p-[12px] flex"
                    style={{
                      background: 'var(--teal-deep)',
                      boxShadow: isLeader
                        ? '0 10px 28px rgba(140,42,84,0.3), 0 0 0 2px var(--gold-hi)'
                    ...
                    <AnimatePresence>
                      {isLeader && (
                        <motion.div
                          ...
                          className="absolute -top-[12px] -right-[8px] z-20 px-[10px] py-[4px] rounded-full text-[10px] tracking-[1px] uppercase font-bold text-[var(--cream-hi)]"
                          style={{ background: 'var(--berry-deep)', border: '1.5px solid var(--gold-hi)' }}
                        >★ Leading</motion.div>
                      )}
                    </AnimatePresence>

                    <div className="relative z-[1] flex flex-col text-center w-full rounded-[8px] px-[14px] pt-[16px] pb-[16px] overflow-hidden"
                      style={{ background: card.bg, boxShadow: 'inset 0 0 0 1px rgba(28,13,12,0.06)' }}>
                      <div className="absolute top-2 bottom-2 left-[5px] w-[10px] punch-holes opacity-50" aria-hidden="true" />
                      <div className="absolute top-2 bottom-2 right-[5px] w-[10px] punch-holes opacity-50" aria-hidden="true" />

                      <img loading="lazy" decoding="async" src={card.img} alt="" aria-hidden="true"
                        className="mx-auto w-[clamp(72px,42%,108px)] h-auto object-contain mb-[4px]"
                        style={{ filter: 'drop-shadow(0 3px 5px rgba(28,13,12,0.28))' }} />

                      <h4 className="font-normal text-[18px] mt-[4px] mb-[2px] text-[var(--cocoa)] leading-snug" style={macklin}>{card.name}</h4>
                      <div className="font-script-alt text-[15px] text-[var(--berry)] mb-[14px] leading-snug">{card.note}</div>
                          ...
                          {Array.from({ length: 10 }).map((_, s) => {
                            ...
                            return (
                              <motion.span key={s} className="absolute text-[13px]"
                                ...
                                animate={{ opacity: 0, x: Math.cos(ang) * 70, y: Math.sin(ang) * 70, scale: 1.1 }}
                        ...
                        {!revealed ? (
                          ...
                          <div className="w-full">
                            ...
                            <div className="flex items-baseline justify-center gap-2 mb-[8px]">
                              <span className="text-[28px] leading-none text-[var(--cocoa)]" style={macklin}>
                                <CountUp to={pct} reduce={reduce} suffix="%" />
                              </span>
                            </div>
                            <div className="h-[7px] bg-[rgba(28,13,12,0.12)] rounded-full overflow-hidden">
            ...
            <motion.div
              ...
              viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.3 }}
              className="mt-[20px] text-[12px] tracking-[2px] uppercase font-semibold text-[var(--cocoa)] opacity-45"
